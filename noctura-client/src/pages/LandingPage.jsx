...
export default function LandingPage() {
  ...
  return <div className="landing-page">
    <header className="landing-header"><Brand to="/" /><nav><a href="#features">Features</a><a href="#movies">Movies</a><Link to="/login">Sign in</Link><ThemeSwitcher compact /><Link to="/register" className="button button--primary">Join free</Link></nav></header>
    <section className="landing-hero" style={{ backgroundImage: `linear-gradient(90deg,rgba(5,6,10,.98),rgba(5,6,10,.7),rgba(5,6,10,.25)),linear-gradient(0deg,#08090d,transparent 45%),url(${backdropUrl(hero?.backdrop_path)})` }}>
      <div><span className="eyebrow">Your next favorite movie is here</span><h1>Cinema, curated.<br /><span>Stories, remembered.</span></h1><p>Explore trending releases, build your personal watchlist, save favorites, track your history, and share reviews—all in one beautiful experience.</p><div className="hero-actions"><Link to="/register" className="button button--light"><Play fill="currentColor" /> Start watching</Link><Link to="/login" className="button button--glass">Sign in <ArrowRight /></Link></div></div>
    </section>
    <section className="landing-features" id="features"><div><Search /><h3>Discover instantly</h3><p>Search TMDB’s extensive movie catalogue and browse curated collections.</p></div><div><Heart /><h3>Save your favorites</h3><p>Keep the films you love close and access them from any session.</p></div><div><Bookmark /><h3>Plan what’s next</h3><p>Build a watchlist and maintain a personal viewing history.</p></div><div><Star /><h3>Share your voice</h3><p>Rate and review movies with the Noctura community.</p></div></section>
    ...
    <section className="landing-cta"><span className="eyebrow">Ready for movie night?</span><h2>Your watchlist is waiting.</h2><p>Create your Noctura account and shape a movie universe that feels entirely yours.</p><Link to="/register" className="button button--light">Create free account <ArrowRight /></Link></section>
